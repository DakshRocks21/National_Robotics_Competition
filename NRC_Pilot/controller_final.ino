...
const int max_WASD_speed = 180;
const int  max_Rot_speed = 270;
const int  max_Dia_speed = 180;
...
void onDataRecv(const esp_now_recv_info * info, const uint8_t *controllerData, int len) { 
  ...
  int speedDiagonal = map(max(abs(incomingData.leftX-2047), abs(incomingData.leftY-2047)), 0, 2048, 0, max_Dia_speed);
// changed -2047 to 0
  ...
  // for (int i =0; i<4 ; i++)
  // {
  //   Serial.println(incomingData.individualModules[i]);
  // }


  // INDIVIDUAL MODULES
 if (incomingData.rightX > joy_high_limit) {
    ...
    if (allModulesActive) {
        ...
        for (int i = 0; i < 4; i++) {
            if (incomingData.individualModules[i] == 1) {
                my_robot.HingePivotRight(speedRightX, i);
                Serial.println("Left Turn");
            }
        }
    }
} else if (incomingData.rightX < joy_low_limit) {
    bool allModulesActive = true;
    for (int i = 0; i < 4; i++) {
        if (incomingData.individualModules[i] != 1) {
            allModulesActive = false;
    ...
    if (allModulesActive) {
        ...
        for (int i = 0; i < 4; i++) {
            if (incomingData.individualModules[i] == 1) {
                my_robot.HingePivotLeft(speedRightX, i);
                Serial.println("Right Turn");
            }
        }
    }
  ...
  else if(incomingData.leftX > joy_high_limit && incomingData.leftY > joy_high_limit) {
    my_robot.MoveDiagDownLeft(speedDiagonal);
    Serial.println("DownLeft");
  }
  else if(incomingData.leftX > joy_high_limit && incomingData.leftY < joy_low_limit) {
    my_robot.MoveDiagUpLeft(speedDiagonal);
    Serial.println("UpLeft");
  }
  else if(incomingData.leftX < joy_low_limit && incomingData.leftY > joy_high_limit) {
    my_robot.MoveDiagDownRight(speedDiagonal);
    Serial.println("DownRight");
  }
  else if(incomingData.leftX < joy_low_limit && incomingData.leftY < joy_low_limit) {
    my_robot.MoveDiagUpRight(speedDiagonal);
    Serial.println("UpRight");
  }
  else if(incomingData.leftY > joy_high_limit) {
    my_robot.MoveBackward(speedLeftY);
    Serial.println("Backward");
  }
  else if(incomingData.leftY < joy_low_limit) {
    my_robot.MoveForward(speedLeftY);
    Serial.println("Forward");
  }
  else if(incomingData.leftX < joy_low_limit) {
    my_robot.MoveRight(speedLeftX);
    Serial.println("Right");
  }
  else if(incomingData.leftX > joy_high_limit) {
