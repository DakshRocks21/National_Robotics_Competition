...
void clearnroom(){ 
// for all the move foward/backwards abit, pls add if statement to check if it can move foward/backward
// add if x > 2.5m ignore that value
// ik i only say if left/right/front/back ultrasonic >5mm, add or to detect colour to trigger and end current movent when colour is detected 
  ...
  while true{
    ...
    if  (leftultrasonic > missingx, missingx != 0){
      ...
      while (backultrasonic > 5mm || rightultrasonic <= missing +-x){
        move back or walltracing(backultrasonic);
      }
      if (rightultrasonic <= missing +-x){
      mover forward
      }

      

      while (rightultrasonic < +-x){
        while (rightultrasonic > 5mm){
          move right or walltracing(rightultrasonic);
          if (leftultrasonic > x){
            x = leftultrasonic;
